...
def bot_loop(bot, info_event):
    ...
    This replaces the complex game logic with connection testing
    """
    logger = bot.logger if hasattr(bot, 'logger') else logging.getLogger('bot_handler')
    logger.info("Bot loop started (simplified mode)")
...
def start_bot_class(logger):
    """Create and return bot instance - compatibility function"""
    try:
        return bot_core.Bot(logger=logger)
    except Exception as e:
