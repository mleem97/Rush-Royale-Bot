...
def _resolve_unit_template_path(unit_filename: str) -> Path | None:
    ...
    matches.sort(
        key=lambda p: (
            0 if any(part in preferred_parts for part in p.parts) else 1,
            len(p.parts),
        )
    )
    return matches[0]
...
def select_units(units):
    ...
    for new_unit in units:
        try:
            ...
            if src_path is None:
                print(f"{new_unit} not found under cv-images/all_units/")
                continue
...
def combat_loop(bot, grid_df, mana_targets, user_target="demon_hunter.png"):
    ...
    grid_df, unit_series, merge_series, df_groups, info = bot.try_merge(
        prev_grid=grid_df, merge_target=user_target
    )
