...
class TrainingTabFrame(ctk.CTkFrame):
    ...
    def _refresh_stats(self):
        ...
        if rank_dir.exists():
            ...
            rank_count += sum(
                len(list(sub.glob("*.png"))) for sub in rank_dir.iterdir() if sub.is_dir()
            )
        ...
        if unit_dir.exists():
            unit_count = sum(
                len(list(sub.glob("*.png"))) for sub in unit_dir.iterdir() if sub.is_dir()
            )
    ...
    def _load_next_unlabeled(self):
        ...
        self.status_label.configure(text=f"Loaded ({len(self._unlabeled_files)} remaining)")
    ...
    def _train_rank_model(self):
        ...
        def run_training():
            try:
                ...
                self.app.after(0, lambda e=exc: self.status_label.configure(text=f"❌ Error: {e}"))
            finally:
    ...
    def _prepare_unit_dataset(self):
        ...
        def run_prepare():
            try:
                ...
                self.progress_bar.set(1.0)
                self.app.after(
                    0, lambda: self.status_label.configure(text=f"✅ Created {count} samples")
                )
    ...
    def _export_to_onnx(self, model_type: str):
        ...
        def run_export():
            try:
                ...
                self.progress_bar.set(1.0)
                self.app.after(
                    0, lambda: self.status_label.configure(text=f"✅ Exported to {onnx_path}")
                )
                self.app.after(
                    0, lambda: self.onnx_status.configure(text=f"Last export: {onnx_path}")
                )

            except Exception as exc:
                self.app.after(
                    0, lambda e=exc: self.status_label.configure(text=f"❌ Export error: {e}")
                )
            finally:
                self.app.after(500, lambda: self.progress_bar.set(0))
