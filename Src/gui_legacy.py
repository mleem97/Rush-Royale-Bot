...
class RushBot:
    def __init__(self):
        ...
        self.ads_var, self.pve_var, self.unit_update_var, self.mana_vars, self.floor = (
            create_options(self.frames[0], self.config)
        )
        ...
        fg = "#ffffff"
        logger_feed = Text(
            self.frames[3], height=30, width=38, bg=bg, fg=fg, wrap=WORD, font=("Consolas", 9)
        )
        ...
        leave_dungeon = Button(
            self.frames[2], text="Quit Floor", command=self.leave_game, bg="#ff0000", fg="#000000"
        )
...
def create_options(frame1, config):
    ...
    ads_var = IntVar()
    pve_check = Checkbutton(frame1, text="PvE", variable=pve_var, justify=LEFT).grid(
        row=0, column=1, sticky=W
    )
    unit_update_check = Checkbutton(
        frame1, text="Unit Update", variable=unit_update_var, justify=LEFT
    ).grid(row=0, column=2, sticky=W)
    # ad_check = Checkbutton(frame1, text='Watch ads', variable=ads_var,justify=LEFT).grid(row=0, column=2, sticky=W)
    # Mana level targets
    mana_label = Label(frame1, text="Mana Level Targets", justify=LEFT).grid(
    ...
    mana_buttons = [
        Checkbutton(frame1, text=f"Card {i + 1}", variable=mana_vars[i], justify=LEFT).grid(
            row=2, column=i + 1
        )
        for i in range(5)
    ]
