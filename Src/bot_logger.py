...
class TextHandler(logging.StreamHandler):
    ...
    def __init__(self, textctrl):
        ...
        self.ansi_colors_dark = ['black', 'red', 'green', 'yellow', 'royal blue', 
            'magenta', 'cyan', 'light gray']
        self.ansi_colors_light = [
            'dark gray', 'tomato', 'light green', 'light goldenrod', 'light blue', 
                'pink', 'light cyan', 'white'
        ]
        # regular expressionto find ansi codes in string
        self.ansi_regexp = re.compile(r"\x1b\[((\d+;)*\d+)m")

        self.textctrl.tag_configure('foreground default', 
            foreground=self.textctrl['fg'])
        self.textctrl.tag_configure('background default', 
            background=self.textctrl['bg'])
        for i, (col_dark, col_light) in 
            enumerate(zip(self.ansi_colors_dark, self.ansi_colors_light)):
            ...
            # tag configuration
            self.textctrl.tag_configure('foreground ' + 
                col_dark, foreground=col_dark)
            self.textctrl.tag_configure('background ' + 
                col_dark, background=col_dark)
            self.textctrl.tag_configure('foreground ' + 
                col_light, foreground=col_light)
            self.textctrl.tag_configure('background ' + 
